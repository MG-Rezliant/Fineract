...
public class JobStarter {
    ...
    public JobExecution run(Job job, ScheduledJobDetail scheduledJobDetail, Set<JobParameterDTO> jobParameterDTOSet,
        ...
        try {
            ...
            JobParameters jobParameters = new JobParametersBuilder(getNextJobParameters(job))
                    ...
                    .toJobParameters();
            // Batch 6 JobOperator.start() silently discards explicit parameters for jobs that define an
            // incrementer and launches the next instance instead; the incrementer is already applied in
            // getNextJobParameters(), so use the parameter-respecting launch inherited from JobLauncher
