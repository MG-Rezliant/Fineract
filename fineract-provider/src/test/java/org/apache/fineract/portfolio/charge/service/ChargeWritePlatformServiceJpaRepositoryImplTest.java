...
/**
 * Regression coverage for FINERACT-2752 (CBS-129): {@code updateCharge} used a plain {@code save()}, so an
 * edited charge (e.g. a changed "Income from Charge" GL account) wasn't flushed before the same request
 * re-read the charge to build the response, making the update look reverted/missing on an immediate re-fetch.
 */
