...
public final class WorkingCapitalLoanPeriodPaymentRateHistoryHelper {
    ...
    /**
     * The rate the schedule was originally generated at: the head of the active chain records it as its
     * {@code previousRate}, because {@link #restatePreviousRates} assigns exactly that.
     *
     * <p>
     * Reversed changes are ignored. They would answer the same for a loan whose history was only ever appended to, but
     * a reversed change is never restated, so once one is superseded by a backdated correction its recorded predecessor
     * can be stale — and the chain head is authoritative either way.
     *
     * @param changes
     *            the loan's rate changes, in any order; reversed ones are ignored, so passing only the active ones is
     *            equivalent
     * @param loanCurrentRate
     *            the loan's own rate, returned when no change is active (with none applied it is the original, and with
     *            every one reversed the loan has been restored to it)
     */
    public static BigDecimal originalRate(final List<WorkingCapitalLoanPeriodPaymentRateChange> changes, final BigDecimal loanCurrentRate) {
        return changes.stream().filter(change -> !change.isReversed()).min(CHRONOLOGICAL)
                .map(WorkingCapitalLoanPeriodPaymentRateChange::getPreviousRate).orElse(loanCurrentRate);
    }

    /**
     * The rate in force on {@code asOf}: the latest active change effective on or before that date, falling back to
     * {@link #originalRate} when none has taken effect yet.
     *
     * <p>
     ...
     * @param loanCurrentRate
     *            see {@link #originalRate}
     */
    public static BigDecimal rateInEffectAt(final List<WorkingCapitalLoanPeriodPaymentRateChange> changes, final BigDecimal loanCurrentRate,
            final LocalDate asOf) {
        return changes.stream().filter(change -> !change.isReversed()).filter(change -> !change.getEffectiveDate().isAfter(asOf))
                .max(CHRONOLOGICAL).map(WorkingCapitalLoanPeriodPaymentRateChange::getNewRate)
                .orElseGet(() -> originalRate(changes, loanCurrentRate));
    }
     ...
     * @param originalRate
     *            the rate the schedule was generated at, resolved via {@link #originalRate} <strong>before</strong> any
     *            rewriting: this method overwrites the very field that one reads, so a value derived afterwards would
     *            feed on its own output and could drift the base rate every schedule is rebuilt from
