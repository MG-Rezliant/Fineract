...
public class WorkingCapitalLoanAmortizationScheduleWriteServiceImpl implements WorkingCapitalLoanAmortizationScheduleWriteService {
    ...
    private ProjectedAmortizationScheduleModel reconstructScheduleModel(final WorkingCapitalLoan loan,
        ...
        // One fetch serves both uses: the earliest of these carries the rate the schedule was generated at, and they
        // are also the changes to replay onto it.
    ...
    /**
     * The rate the schedule was originally generated at. Ranked by effective date rather than by insertion order:
     * backdating means the earliest-effective change is not necessarily the earliest-created one, and it is the
     * earliest-effective change whose {@code previousRate} holds the original rate.
     *
     * @param rateChanges
     *            every rate change on the loan, reversed ones included
     */
    private BigDecimal resolveOriginalPeriodPaymentRate(final WorkingCapitalLoan loan,
            final List<WorkingCapitalLoanPeriodPaymentRateChange> rateChanges) {
        // Left null when the product details are missing, matching how generateBaseModel treats its other
        // product-detail reads.
        final BigDecimal loanCurrentRate = loan.getLoanProductRelatedDetails() != null
                ? loan.getLoanProductRelatedDetails().getPeriodPaymentRate()
                : null;
        return WorkingCapitalLoanPeriodPaymentRateHistoryHelper.originalRate(rateChanges, loanCurrentRate);
    }
