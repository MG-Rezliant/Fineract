...
class ProgressiveEMICalculatorTest {
    ...
    public void test_disbursementAfterLastPeriodFullyPaid_zeroInterest_keepsPeriodsAbovePaidAmounts() {
        ...
        Assertions.assertFalse(emiCalculator.alignPeriodsWithPaidAmounts(interestSchedule, installments, firstFromDate));
        Assertions.assertEquals(0.0, toDouble(interestSchedule.repaymentPeriods().get(2).getPaidPrincipal()), 0.001,
                "An aligning which corrects nothing must not change the model");
        ...
        Assertions.assertTrue(emiCalculator.alignPeriodsWithPaidAmounts(interestSchedule, installments, secondDisbursementDate));

        for (RepaymentPeriod repaymentPeriod : interestSchedule.repaymentPeriods()) {
            Assertions.assertTrue(!repaymentPeriod.getDuePrincipal().isLessThan(repaymentPeriod.getPaidPrincipal()),
                    "Period %s has less due principal (%s) than what has been paid on it (%s)".formatted(repaymentPeriod.getDueDate(),
                            repaymentPeriod.getDuePrincipal(), repaymentPeriod.getPaidPrincipal()));
        }
